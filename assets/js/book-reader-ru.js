document.addEventListener('DOMContentLoaded', () => {
  ...
  function milestone() {
    ...
    [['Book_RU_25',.25],['Book_RU_50',.50],['Book_RU_75',.75]].forEach(([n,t])=>{
      if (pct >= t && !fired.has(n)) { fired.add(n); track(n,{page:current,total}); }
    });
    if (current === total && !fired.has('Book_RU_Finished')) {
      fired.add('Book_RU_Finished'); track('Book_RU_Finished',{page:current,total});
    }
  }
  function render(scroll=false) {
    img.src = srcFor(current);
    img.alt = `Страница ${current} / ${total}`;
